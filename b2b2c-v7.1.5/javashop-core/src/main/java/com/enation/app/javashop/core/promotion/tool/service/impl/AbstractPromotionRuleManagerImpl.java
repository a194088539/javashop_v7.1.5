...
public abstract class AbstractPromotionRuleManagerImpl {
    ...
    protected void verifyName(String name,boolean isUpdate,Integer activeId) {
        ...
        if(isUpdate){
            //判断活动重名
            if (this.daoSupport.queryForInt("select count(0) from es_groupbuy_active where act_name=? and act_id != ? and delete_status = ?", name, activeId, DeleteStatusEnum.NORMAL.value()) > 0) {
                throw new ServiceException(PromotionErrorCode.E402.code(), "当前活动重名，请修正");
            }
        }else {
            //判断活动重名
            if (this.daoSupport.queryForInt("select count(0) from es_groupbuy_active where act_name=? and delete_status = ?", name, DeleteStatusEnum.NORMAL.value()) > 0) {
                throw new ServiceException(PromotionErrorCode.E402.code(), "当前活动重名，请修正");
            }
        }
    }
    ...
    protected void verifyTime(long startTime, long endTime, PromotionTypeEnum typeEnum, Integer activityId) {
        ...
        switch (typeEnum) {
            case HALF_PRICE:
                sql = "select count(0) from es_half_price where ((start_time <= ? and ? <= end_time ) or (start_time <= ? and ? <= end_time )) and seller_id =?";
                params.add(startTime);
                params.add(startTime);
                params.add(endTime);
                params.add(endTime);
                params.add(UserContext.getSeller().getSellerId());
                if (activityId != null) {
                    sql += " and hp_id != ?";
            ...
            case MINUS:
                sql = "select count(0) from es_minus where ((start_time <= ? and ? <= end_time ) or (start_time <= ? and ? <= end_time )) and seller_id =?";
                params.add(startTime);
                params.add(startTime);
                params.add(endTime);
                params.add(endTime);
            ...
            case FULL_DISCOUNT:
                sql = "select count(0) from es_full_discount where ((start_time <= ? and ? <= end_time ) or (start_time <= ? and ? <= end_time )) and seller_id =?";
                params.add(startTime);
                params.add(startTime);
                params.add(endTime);
                params.add(endTime);
            ...
            case GROUPBUY:
                sql = "select count(0) from es_groupbuy_active where delete_status = ? and ((start_time <= ? and ? <= end_time ) or (start_time <= ? and ? <= end_time ))";
                params.add(DeleteStatusEnum.NORMAL.value());
                params.add(startTime);
                params.add(startTime);
                params.add(endTime);
                params.add(endTime);
