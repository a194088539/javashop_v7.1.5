...
public class PintuanGoodsManagerImpl implements PintuanGoodsManager {
    ...
    @Override
    public PinTuanGoodsVO getDetail(Integer skuId,Long time) {
        ...
        List<PinTuanGoodsVO> list = tradeDaoSupport.queryForList(sql, PinTuanGoodsVO.class, skuId, now, time);
    ...
    @Override
    public void deletePinTuanGoods(Integer goodsId) {
        this.tradeDaoSupport.execute("delete from es_pintuan_goods where id in (select t.id from  (select pg.id from es_pintuan_goods pg left join es_pintuan p on pg.pintuan_id = p.promotion_id where pg.goods_id = ? and p.end_time >= ?) as t)",goodsId,DateUtil.getDateline());
    }
}
