...
public class CartPriceCalculatorImpl implements CartPriceCalculator {
    ...
    public PriceDetailVO countPrice(List<CartVO> cartList, CouponVO siteCouponVO) {
        ...
        //根据规则计算价格
        PriceDetailVO priceDetailVO = this.countPriceWithRule(cartList, siteCouponVO);
    ...
    private PriceDetailVO countPriceWithRule(List<CartVO> cartList, CouponVO siteCouponVO) {
        ...
        if (siteCouponVO != null) {
            price.setCouponPrice(siteCouponVO.getAmount());
            price.setTotalPrice(CurrencyUtil.sub(price.getTotalPrice(),siteCouponVO.getAmount()));
        }
    ...
    private void applyCartRule(PromotionRule cartRule, PriceDetailVO cartPrice, CartVO cart) {
        ...
        if (cartRule.getUseCoupon() != null) {
            ...
            cartPrice.setGoodsPrice(subtotal);
            //要使用的优惠券
            CouponVO useCoupon = cartRule.getUseCoupon();
            //查看是平台优惠券，还是商家优惠券
            if (useCoupon.getSellerId().equals(0)) {
