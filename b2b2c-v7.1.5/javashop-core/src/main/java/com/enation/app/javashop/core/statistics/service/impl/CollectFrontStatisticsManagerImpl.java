...
public class CollectFrontStatisticsManagerImpl implements CollectFrontStatisticsManager {
     ...
     */
    @Override
    public SimpleChart getChart(Integer sellerId) {

        // 从es_sss_goods_data表中查询，商品名称，收藏数量
        String sql = " SELECT goods_id,favorite_num,goods_name FROM es_sss_goods_data WHERE seller_id = ? ORDER BY favorite_num DESC LIMIT 50 ";
     ...
     */
    @Override
    public Page getPage(Integer pageNo, Integer pageSize, Integer sellerId) {

        // 获取商品名，收藏数量，商品价格的正在出售的商品，按收藏数量排序
        String sql = "select goods_id,goods_name,favorite_num,price from es_sss_goods_data where seller_id = ?" +
                " and market_enable = 1 order by favorite_num desc";

        return this.daoSupport.queryForPage(sql, pageNo, pageSize, sellerId);
    }
