...
public class MemberSecurityBuyerController {
    ...
    public String sendBindEmailCode(@NotEmpty(message = "uuid不能为空") String uuid, @NotEmpty(message = "图片验证码不能为空") String captcha, @NotEmpty(message = "电子邮箱不能为空") String email) {
        boolean isPass = captchaClient.valid(uuid, captcha, SceneType.BIND_EMAIL.name());
        if (!isPass) {
            throw new ServiceException(MemberErrorCode.E107.code(), "图片验证码不正确");
        }
        //清除除图片验证码信息
        captchaClient.deleteCode(uuid, captcha, SceneType.BIND_EMAIL.name());
