<template>
  <div class="bg-settlement">
    ...
    <div class="sum-settlement">
      ...
      <p>
        <span>本期应收金额：</span>
        <span><strong style="color: red;font-size: 18px;">{{ settlementData.bill_price | unitPrice('￥') }}</strong> = {{ settlementData.order_total_price | unitPrice('￥') }} (订单收入总金额) - {{ settlementData.refund_total_price | unitPrice('￥') }} (订单退还总金额) -
          ...
          {{ settlementData.distribution_rebate | unitPrice('￥') }} (分销退还佣金总金额)</span>
      </p>
      <p>
        <span>结算状态：</span>
