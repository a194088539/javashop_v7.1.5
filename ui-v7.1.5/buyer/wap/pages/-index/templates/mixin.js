...
export default {
  ...
  components: {
    'layout-item': {
      props: ['block', 'isEdit', 'isBanner', 'blockHref', 'isMiniPrograme'],
      template: `<div class="layout-item">
                   ...
                   <template v-else >
                    <template  v-if="isMiniPrograme">
                      <span v-if="!isBanner" @click="$parent.blockHref(block)">
                       <slot :block="block"></slot>
                       <img v-if="block.block_type === 'IMAGE'" :src="block.block_value">
                     </span>
                     <span v-if="isBanner" @click="blockHref(block)">
                       <slot :block="block"></slot>
                       <img v-if="block.block_type === 'IMAGE'" :src="block.block_value">
                     </span>    
                    </template>
  ...
  methods: {
    ...
    blockHref(block) {
      ...
      if (this.isMiniPrograme) {
        ...
        switch (opt_type) {
          // 链接地址 
          case 'URL': wx.miniProgram.navigateTo({ url: `/pages${opt_value}${opt_value}` })
            break
          // 商品
          case 'GOODS': wx.miniProgram.navigateTo({ url: `/pages/goods/goods?goods_id=${opt_value}` })
            break
          // 关键字 跳转至商品搜索页面 keyword
          case 'KEYWORD':  wx.miniProgram.navigateTo({ url: `/pages/category/category?keyword=${opt_value}` })
            break
          // 店铺
          case 'SHOP': wx.miniProgram.navigateTo({ url: `/pages/shop/shop_id/shop_id?id=${opt_value}` })
            break
          // 分类
          case 'CATEGORY': wx.miniProgram.navigateTo({ url: `/pages/category/category?category=${opt_value}` })
            break
